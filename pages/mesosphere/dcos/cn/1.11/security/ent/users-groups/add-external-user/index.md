...
- 外部 [LDAP 目录](/mesosphere/dcos/cn/1.11/security/ent/ldap/)。
- [OpenID Connect 或 SAML 提供程序](/mesosphere/dcos/cn/1.11/security/ent/sso/)。
...
<p class="message--important"><strong>重要信息: </strong>LDAP 目录中的组条目必须使用 <tt>member</tt>、<tt>uniquemember</tt> 或 <tt>memberuid</tt> 属性列出其成员。</p>
...
1. 单击 **Group Import (Optional)**。

1. 为应在 **GROUP SEARCH BASE** 字段中搜索的目录树子集提供 DN。例如：`(cn=Users,dc=mesosphere,dc=com)`。

1. 在 **GROUP SEARCH FILTER TEMPLATE** 字段中提供用于将组名称转换为有效 LDAP 搜索筛选器的模板。字符串必须包含 `%(groupname)`。例如：`(&(objectclass=group)(sAMAccountName=%(groupname)s))`。

1. 完成后，对话框应如下所示。

    ![LDAP 组导入配置](/mesosphere/dcos/cn/1.11/img/ldap-group-import.png)
...
您可以使用 `/ldap/importuser` [IAM API](/mesosphere/dcos/cn/1.11/security/ent/iam-api/) 端点导入一组 LDAP 用户。
...
- 如果您[安全模式](/mesosphere/dcos/cn/1.11/security/ent/#security-modes)为 `permissive` 或 `strict`，则在本部分中发出 curl 命令之前，必须遵循[获取 DC/OS CA 根证书]中的步骤(/mesosphere/dcos/cn/1.11/security/ent/tls-ssl/get-cert/)。如果您的[安全模式](/mesosphere/dcos/cn/1.11/security/ent/#security-modes)为 `disabled`，则必须在将其发出前从命令中删除 `--cacert dcos-ca.crt`。
