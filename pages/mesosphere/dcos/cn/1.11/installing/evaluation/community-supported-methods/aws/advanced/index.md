...
高级 AWS CloudFormation 模板为创建和扩展 DC/OS 群集带来了优势和灵活性。通过这些模板，您可以从一整套 DC/OS 配置选项中进行选择。
 ...
 - 通过添加更多的 [代理](/mesosphere/dcos/1.11/overview/concepts/#agent) 节点来扩展和更新现有的 DC/OS 群集。
...
- 符合 bootstrap 节点 [系统要求] 的节点(/mesosphere/dcos/cn/1.11/installing/production/system-requirements/)。
...
有关参数描述和配置示例，请参阅 [文档](/mesosphere/dcos/cn/1.11/installing/production/advanced-configuration/configuration-reference/)。
...
4. 转到 [S3](https://console.aws.amazon.com/s3/home) 并导航至上述 `<path-to-directory>` 中的 s3 bucket.
...
- 在 **选择模板** 页面，从您的工作站上传 [Zen](/mesosphere/dcos/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#zen) 模板，然后单击 **下一步**。
...
2. 在 **指定详情**页面指定这些值，然后单击 **下一步**。
    ...
    ![AWS UI](/mesosphere/dcos/cn/1.11/img/aws-advanced-1.png)

    图 1. AWS Advanced Web 界面

    * **堆栈名称** 指定群集名称。
    * **CustomAMI** 可选：指定 AMI ID。如需更多信息，请参阅 [使用自定义 AMI 进行安装](/mesosphere/dcos/cn/1.11/installing/production/system-requirements/custom-ami/)。
...
* 群集堆栈启动需要 15 到 20 分钟时间。您将为其逐个创建堆栈，其中 `<stack-name>` 是您为 **Stack name** 确定的值，而 `<stack-id>` 是自动生成的 ID。

 ![AWS UI](/mesosphere/dcos/cn/1.11/img/aws-advanced-2.png)
 ...
 **注意：** 可能需要调整窗口大小以查看此选项卡。可以随时在<a href="https://console.aws.amazon.com/cloudformation/home" target="_blank"> AWS CloudFormation 管理</a>页面找到 DC / OS 主机名。
 ...
 ![Monitor stack creation](/mesosphere/dcos/cn/1.11/img/dcos-aws-step3a.png)
...
1. 单击 **登录到 DC/OS**。
 ...
 ![UI installer success](/mesosphere/dcos/cn/1.11/img/gui-installer-success-ee.gif)
 ...
 **注意：** 默认用户名为 `bootstrapuser`，默认密码为 `deleteme`。
 ...
 ![alt text](/mesosphere/dcos/cn/1.11/img/ui-installer-auth2.png)
...
可以通过创建新的堆栈来添加更多代理节点。使用 [advanced-priv-agent.json](/mesosphere/dcos/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#private-agents) 或 [advanced-pub-agent.json](/mesosphere/dcos/cn/1.11/installing/evaluation/community-supported-methods/aws/advanced/template-reference/#public-agents) 模板。这些模板创建代理，然后作为 AutoScalingGroup 的一部分附加到 `PrivateAgentStack` 或 `PublicAgentStack`。
...
对于所有高级配置选项，请参阅模板参考 [文档](/mesosphere/dcos/cn/1.11/installing/evaluation/cloud-installation/aws/advanced/template-reference/)。
...
# 模板参考
如需完整的高级配置选项，请参阅模板参考 [文档](/mesosphere/dcos/cn/1.11/installing/evaluation/cloud-installation/aws/advanced/template-reference/)。
